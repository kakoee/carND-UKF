...
std::vector<double> UKF::CalculatePrediction(std::vector<double> sp,double dt){
    if(sp.size()!=7)
        return {0,0,0,0,0};
    vector<double> spred;    
    double px= sp[0];
    ...
    double v_sik = sp[6];
    if(sydot>0.001){
        px= px+ (sy/sydot)*(sin(sy+dt*sydot)-sin(sy))  + ;
        py= py + (sy/sydot)*(-cos(sy_dt*sydot)+cos(sy));
        v=v+0;
        sy = sy + dt*sydot;
        sydot=sydot+0;
    }
    ...
    px= px + 0.5*(dt*dt) * cos(sy)* v_ak;
    py = py+ 0.5 *(dt*dt) * sin(sy)*v_ak;
    v = v + dt *v_ak;
    sy = sy  + 0.5 * (dt*dt) * v_sik;
    sydot = sydot + dt * v_sik;
    spred.push_back(px);
    spred.push_back(py);
    spred.push_back(v);
    spred.push_back(sy);
    spred.push_back(sydot);
    return spred;
 ...
 ******************************************************************************/
  ...
  vector<double> spoint_pred;
  for(int i = 0;i<2*n_aug+1;i++){
      for(int j=0;j<7;j++){
          spoint.push_back(Xsig_aug(i,j));
      }
      spoint_pred=CalculatePrediction(spoint,delta_t);
      Xsig_pred.col(i) << spoint_pred[0],spoint_pred[1],spoint_pred[2],spoint_pred[3],spoint_pred[4];
...
void UKF::CalculatePrediction(std::vector<double>& sp,double dt,std::vector<double>& spred){
    ...
    std::cout<< sydot << std::endl;
    std::cout<< v_ak << std::endl;
    std::cout<< v_sik << std::endl;
    */
 ...
 ******************************************************************************/
  ...
  //predict sigma points
  std::vector<double> spoint(7);
  std::vector<double> spoint_pred(5);
  for(int i = 0;i<2*n_aug+1;i++){
      for(int j=0;j<7;j++){
